...
本章の目的は、以降の各章で繰り返し登場する判断の土台（抽象化/具体化、トレードオフ評価、負債評価、AI活用の境界）を共通言語として揃えることである。
...
- 人間の判断が必要: ビジネス価値の評価、制約条件の優先順位、最終的な意思決定
...
AI時代の思考OSでは、「AIを使うかどうか」だけでは不十分である。実務では、AIに委任した作業、人間が検証する作業、人間が最終責任を持つ判断を分けて管理する必要がある。

| 層 | 主な問い | 具体例 | 成果物 |
| --- | --- | --- | --- |
| Delegate（委任） | AIに任せてよい作業か | 代替案の列挙、レビュー観点の洗い出し、ドラフト作成 | プロンプト、入力範囲、出力形式 |
| Review（検証） | 採用前に何で裏取りするか | 一次情報確認、テスト実行、セキュリティレビュー、再現手順確認 | 検証結果、採否理由、レビュー記録 |
| Own（責任） | 誰が最終判断し、失敗時にどう説明するか | リスク受容、リリース判断、本番操作、顧客説明 | ADR、承認ログ、Runbook、ロールバック計画 |

この3層を分けないままAIを導入すると、「速く作れたが誰も検証していない」「レビューが形骸化した」「障害時に判断理由を説明できない」という問題が起きる。AIの利用範囲を広げるほど、判断責任の所在と検証証跡を明確にする必要がある。

実務では、[AI協働の標準手順（SOP）](../../introduction/ai-collaboration-sop/) の実務適用ゲートと、[付録AのAI活用判断メモ](../../appendices/templates/) を併用すると、日々の判断をPR、ADR、Runbookへ接続しやすい。
...
- 生成AIを含む各種ツールを前提に、「AIに任せる部分」と「人間が判断すべき部分」を切り分けるための土台となる認知フレームを提示した。
...
- [ ] AIに委任した作業、人間が検証した証跡、人間が責任を持つ判断を分けて説明できるか。
