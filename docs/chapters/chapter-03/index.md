...
ここで扱うオプション価値は、金融工学の厳密な定義というよりも、「今すぐは使わないかもしれないが、将来の選択肢を残しておくこと自体に価値がある」という直感的な考え方として捉える。たとえば、マイクロサービス移行を見据えた疎結合な設計や、特定クラウドベンダーへの依存度を下げる構成などは、「将来の選択肢（オプション）に対してコストを払っておく」設計判断の典型例である。
...
- アーキテクチャ設計の実例や判断プロセスの詳細は、[付録B：ケーススタディ](../../appendices/case-studies.md) の関連ケースが参考になる。*** End Patch
