...
本章の目的は、不確実性とトレードオフを前提に、評価軸・根拠・意思決定ログを揃えて設計判断を下すプロセスを身につけることである。
...
これらは「組織の責任境界」と「変更の単位（モジュール/サービス）」を揃えるための考え方である。実務では、最低限次を確認する。

- 所有者（Owner）と変更権限が明確か（誰が決め、誰が直すか）
- 変更のたびに複数チームの同時調整が必要になっていないか（調整コストがボトルネック化していないか）
- インターフェース（API/イベント/契約）が文書化され、テスト可能か

**実務例**：
- 重要機能の変更が常に複数チームの同時調整になる場合、境界（責任分界）とインターフェースの見直しを検討する。
- 望ましい境界に合わせて組織を変えられない場合は、まず境界と契約（API仕様・データ契約）を固定し、調整コストを可視化する。
...
**要点**：
- 不確実性（技術/ビジネス/組織）を分解し、「将来の選択肢を残す設計」を投資として評価する。
- 技術選定は、評価軸・重み・根拠を明示したマトリクスで比較し、判断の再現性を高める。
- LLM/生成AIを組み込む場合は、ガードレール・評価・監査・コスト/レイテンシ等をアーキテクチャ要件に組み込む。
